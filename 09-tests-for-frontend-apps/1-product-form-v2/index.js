...
export default class ProductForm {
  ...
  buildImagesList() {
    ...
    const listItems = this.formData.images.map(image =>
      this.getImageItem(image.url, image.source));

    const sortableList = new SortableList({ items: listItems });
  ...
  getSubCategoriesTemplate() {
    ...
    this.categories.forEach(c => {
      options.push(...c.subcategories.map(sc =>
        new Option(`${c.title} > ${sc.title}`, `${sc.id}`).outerHTML
      ));
    });
